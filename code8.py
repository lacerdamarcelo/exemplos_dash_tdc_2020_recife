...
def plot_map(selectedData):
    if selectedData is None:
        region_filter = regions
    else:
        region_filter = [selected_dict['y'] for selected_dict in selectedData['points']]
    selected_data = data[data['region'].isin(region_filter)][['latitude', 'longitude',
                                                              'brightness']]
    map_data = [go.Densitymapbox(lat=selected_data['latitude'], lon=selected_data['longitude'],
                                 z=selected_data['brightness'], radius=10)]
    center = {'lat': selected_data['latitude'].mean(),
              'lon': selected_data['longitude'].mean()}
